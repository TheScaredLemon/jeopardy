...
function fillTable() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      for (let j = 0; j < categories.length; j++) {
        ...
        const value = (i + 1) * 100; // Calculate the value for the clue
        const cell = $("<td>")
          .addClass("clue")
          .attr("data-row", i)
          .attr("data-cat", category.title)
          .text(value); // Assign the value to the clue cell
        row.append(cell);
      }
      tbody.append(row);
    }
  ...
  function handleClick(evt) {
    ...
    if (!clue.showing) {
      ...
      answer.find("i").remove(); // Remove any <i> elements from the answer
